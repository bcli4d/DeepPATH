...
""" ISB modifications:
    ...
    Modifications:
        -- Determine maximum image buffer needed across all files. Create such a buffer when starting a new file.
        -- Only write output after the last tile of input file is processed.
        ...
        with open(FLAGS.tiles_stats) as f:
                for line in f:
                        ...
                        if len(line2)>0:        
                                #print(line2)
                                #print(len(line2))
                                basename = line2[0].split('.')[:-1]
                                stats_dict['.'.join(basename)] = line
                                c,r = basename[1].split('_')[1:3]
                                rmax = max(rmax, int(r))
                                cmax = max(cmax, int(c))
                                
        return stats_dict, rmax, cmax
...
def check_file_done(cTileRootName, NewSlide, dir_name):
        ...
        print("Checking for existence of {}".format(filename))
        if os.path.isfile(filename):
            print(filename + " has already been processed. Skipping.")
            skip = True
        else:
            skip = False
        return skip
...
def main():
  ...
  ##    Average score for each class (first sum then divide)
        ...
        stats_dict, rmax, cmax  = dict_tiles_stats()

        print("rmax: {}, cmax: {}".format(rmax, cmax))
        bufferx = (cmax+2) * (FLAGS.tiles_size - FLAGS.tiles_overlap)
        buffery = (rmax+2) * (FLAGS.tiles_size - FLAGS.tiles_overlap)
        if FLAGS.resample_factor > 0:
            bufferx = int(bufferx / FLAGS.resample_factor)
            buffery = int(buffery / FLAGS.resample_factor)
        
        print("max ypixels: {}, max xpixels {}".format(buffery, bufferx))
        ...
        for tile in sorted(filtered_dict.keys()):
                ...
                if cTileRootName == SlideRootName:
                        ...
                        skip = check_file_done(SlideRootName, NewSlide, dir_name)
                        if skip:
                            continue
                        ...
                        WholeSlide_0 = np.zeros([bufferx, buffery, 3])
                        HeatMap_0 = np.zeros([bufferx, buffery, 3])
                        HeatMap_divider = np.zeros([bufferx, buffery, 3])
                ...
                max_xpixel = max(max_xpixel, req_xLength)
                max_ypixel = max(max_ypixel, req_yLength)
                
                if (req_xLength > bufferx):
                    print("req_xLength: {},  bufferx: {}".format(req_xLength, bufferx))
                    assert(req_xLength <= bufferx)
                    #continue
                if (req_yLength > buffery):
                    print("req_yLength: {},  buffery: {}".format(req_yLength, buffery))
                    assert(req_yLength <= buffery)
                    #continue
        ...
        # Trim off the area that we don't need
        HeatMap_0 = HeatMap_0[0:max_xpixel+1, 0:max_ypixel+1,:]
        HeatMap_divider = HeatMap_divider[0:max_xpixel+1, 0:max_ypixel+1,:]
        WholeSlide_0 = WholeSlide_0[0:max_xpixel+1, 0:max_ypixel+1,:]
